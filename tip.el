  ...
  (if tip-mode
      (progn
        ...
        (when (and (locate-library "flymake")
                   (not (bound-and-true-p tip-flymake-mode)))
          (tip-flymake-mode 1))
        ;; Eldoc: contribute error message at point.
        (add-hook 'eldoc-documentation-functions #'tip--eldoc-error nil t)
...
;;; * inline error display + navigation + eldoc + Flymake
...
Also picks up warning-severity overlays — callers filter if needed."
...
Called after a tip compile response updates error overlays so the
Flymake diagnostic list stays in sync."

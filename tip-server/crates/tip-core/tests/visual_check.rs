...
fn visual_inline_simple() {
    ...
    let out = FragmentCompiler::compile_fragment(&mut world, "$a + b = c$", "#000000", "").unwrap();
    write_svg("inline_simple", &out.svg);
    eprintln!("height: {:.2}pt, depth: {:.2}pt", out.height_pt, out.depth_pt);
}
...
fn visual_colored() {
    let mut world = TipWorld::new();
    let out = FragmentCompiler::compile_fragment(&mut world, "$integral_0^infinity e^(-x^2) dif x = frac(sqrt(pi), 2)$", "#ff3333", "").unwrap();
...
fn visual_with_preamble() {
    ...
    let out = FragmentCompiler::compile_fragment(&mut world, "$cl(RR^n)$", "#000000", preamble).unwrap();

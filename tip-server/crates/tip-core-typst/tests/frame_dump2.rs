...
fn dump(frame: &Frame, depth: usize, y_offset: f64) {
    ...
    eprintln!(
        "{indent}Frame h={:.2} bl={:.2} has_bl={} y_off={:.2}",
        frame.height().to_pt(),
    ...
    for (pos, item) in frame.items() {
        match item {
            FrameItem::Text(t) => {
                eprintln!(
                    "{indent}  Text y={:.3} size={:.1}pt text={:?}",
                    pos.y.to_pt(),
                    t.size.to_pt(),
                    t.text.as_str()
                );
            ...
            FrameItem::Group(g) => {
                eprintln!("{indent}  Group at y={:.3}:", pos.y.to_pt());
...
fn dump_frames_with_and_without_bounded() {
    ...
    for (label, math) in [
        ...
        let src =
            format!("#set page(height: auto, width: auto, margin: 0pt, fill: none)\n{math}\n");

...
fn visual_colored() {
    let mut world = TipWorld::new();
    let out = BottomUpCompiler::compile_fragment(&mut world, "$integral_0^infinity e^(-x^2) dif x = frac(sqrt(pi), 2)$", "#ff3333", "").unwrap();
...
fn visual_with_preamble() {
    ...
    let out = BottomUpCompiler::compile_fragment(&mut world, "$cl(RR^n)$", "#000000", preamble).unwrap();
